<?php
require WP_MCM_PLUGIN_DIR . '/api/class-connection-exception.php';
require_once WP_MCM_PLUGIN_DIR . '/api/class-resource-not-found-exception.php';
class Wp_Mailchimp_Master_v3_Client {
    ...
    private function request( $method, $resource, array $data = array() ) {
        ...
        if( empty( $this->api_key ) ) {
            throw new Wp_Mcm_Exception( "Missing API key", 001 );
        }
         ...
         * @param array $args
         */
        $args = apply_filters( 'wp_mcm_http_request_args', $args, $url );
    ...
    private function get_headers() {
        ...
        $headers['User-Agent'] = 'wp_mcm/' . PLUGIN_NAME_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' );
    ...
    private function parse_response( $response ) {
        ...
        if( $response instanceof WP_Error ) {
            throw new WP_MCM_API_Connection_Exception( $response->get_error_message(), (int) $response->get_error_code() );
        }
        ...
        if( $code >= 400 ) {
            ...
            if( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
                throw new WP_MCM_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
            }

            if( $code === 404 ) {
                throw new Wp_Mailchimp_Master_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
            }

            // mailchimp returned an error..
            throw new Wp_Mcm_Exception( $message, $code, $this->last_request, $this->last_response, $data );
        }
        ...
        throw new Wp_Mcm_Exception( $message, $code, $this->last_request, $this->last_response );
    }
